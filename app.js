...
function renderTeamPage(){const root=document.querySelector("#teamProfile");if(!root)return;const r=rankings.find(x=>x.slug===getParam("team"));if(!r){root.innerHTML=`<main><section class="panel"><h2>Team not found</h2><p>No team matches this link.</p></section></main>`;return}document.title=`${r.team} | CPI`;root.innerHTML=`<section class="hero"><div><p class="kicker">Team Profile · 14U Boys</p><h1>${r.team}</h1><p><a href="${r.clubPage}">${r.club}</a> profile with CPI rank, latest tournament record, and tournament highlight.</p></div><aside class="hero-visual" ${heroStyle(r)}><div class="profile-rank">#${r.postRank}</div><div class="visual-tag"><span>CPI ${Number(r.postCPI).toFixed(1)}</span><span>${moveLabel(r.movement)}</span></div></aside></section><main><section class="profile-grid"><div class="panel">${logoMark(r)}<h2>${r.team}</h2><p class="subtle">Club: <a href="${r.clubPage}">${r.club}</a></p></div><div class="panel"><h2>Latest tournament</h2><div class="stat-strip"><div class="stat"><strong>${r.latestTournamentRecord||"—"}</strong><span>Futures Super Finals record</span></div><div class="stat"><strong>${r.bestWinClean||"—"}</strong><span>Best win</span></div><div class="stat"><strong>${Number(r.postCPI).toFixed(1)}</strong><span>CPI Rating</span></div><div class="stat"><strong>${moveLabel(r.movement)}</strong><span>Movement</span></div></div></div></section></main>`}
